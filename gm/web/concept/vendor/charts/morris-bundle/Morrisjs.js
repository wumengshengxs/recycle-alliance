        ...
        if ($('#morris_line').length) {
            ...
            function data() {
                ...
                for (var x = 0; x <= 30; x += 1) {
                    ret.push({
                        x: x,
                        a: Math.round(Math.random() * 100, 2),
                        b: Math.round(Math.random() * 100, 2),
                        c: Math.round(Math.random() * 100, 2),
                        d: Math.round(Math.random() * 100, 2),
                    });
                }
                return ret;
            }
            var morris_line = Morris.Line({
                ...
                xkey: 'x',
                ykeys: ['a', 'b', 'c', 'd'],
                labels: ['总投递次数', '总投递人数', '总投递重量', '总清运次数'],
                lineColors: ['#ef172c', '#ffc107', '#2ec551', '#25d5f2'],
                hideHover:true,
                parseTime: false
            });
            function update() {
                morris_line.setData(data());
            }
        }
