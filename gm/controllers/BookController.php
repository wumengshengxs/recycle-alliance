 ...
 * Site controller
 */
class BookController extends GController
{
    /**
     * {@inheritdoc}
     */
    ...
    public function actionDetail()
    {
        ...
        $order = SrUserBookOrder::find()
            ...
            ->asArray()->one();
        $order_details = SrUserBookOrderChild::find()
            ->select('*')
            ->where(['book_id' => $order['book_id']])
            ->asArray()->all();

        empty($order_details) && $order_details = [];

        $categorys = SrRubbishCategory::find()
        ...
        foreach ($order_details as &$d) {
            $d['cat_name'] = '未知';
            if (array_key_exists($d['delivery_type'], $category_temp)) {
                $d['cat_name'] = $category_temp[$d['delivery_type']];
            }
            unset($d);
        }
