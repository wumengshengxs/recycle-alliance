...
<body>
    <div class="card" style="margin-bottom:0px">
        ...
        <div class="card-body">
            <div class="row">
                <div class="area-title" style="margin-top: 0;">清运人员姓名</div>
                <input type="text" value="" id='nick_name' class="form-control col-12" />
            </div>
            <div class="row">
                <div class="area-title">清运人员手机</div>
                <input type="number" id="phone_num" value="" class="form-control col-12" maxlength="11"/>
            </div>
            <div class="">
                <div class="area-title">清运权限</div>
                <div class='radio-type'>
                    <label class="radio-inline" style="margin-right: 30px;">
                        <input type="radio" name="type" id="limitCommon" value="0" checked> 普通
                    </label>
                    <label class="radio-inline" style="margin-right: 30px;">
                        <input type="radio" name="type" id="limitSuper" value="1"> 超级权限
                    </label>
                    <label class="radio-inline">
                        <input type="radio" name="type" id="limitMove" value="2"> 机动权限
                    </label>
                </div>
            </div>
            <div class="">
            ...
            <div class="">
                ...
                <ul class='machine list-unstyled'>
                    <?php foreach ($machine as $v) { ?>
                        <li>
                            <label class="checkbox-inline">
                                <input type="checkbox" data-community='<?= $v['community_name'] ?>' data-name="<?= $v['recycler'] ?>" name='machine' value="<?= $v['id'] ?>"> <?= $v['community_name'] ?> — <?= $v['location'] ?>
                            </label>
                        </li>
                    <?php } ?>
...
<script type="text/javascript">
    ...
    $machine.click(function() {
        ...
        for(var j=0;j<name.length;j++){
            if(name[j].split(",")[1]!=''){
                str+='<li>已选设备（'+name[j].split(",")[0]+'）将从清运人员（'+name[j].split(",")[1]+'）管理设备中移除。</li>'
            }
        }
    ...
    function save() {
        ...
        var data = {
            ...
            recycler_status: $('input[name=status]:checked').val().trim(),
