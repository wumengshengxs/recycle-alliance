        ...
        <div class="card" style="margin-bottom:0px">
            <div class="card-header">
                ...
                <div class="row form-group">
                    <input type="text" id="name"  class="form-control col-12" placeholder="请输入维修人员名称" value="<?= !empty($_GET['maintain']) ? $_GET['maintain'] : ''?>" />
                </div>
            </div>
            <div class="card-body">
                <h5></h5>
                <ul class="list-unstyled maintain-ul">
                    <li class="maintain-li maintain-li-active" onclick="maintain(this,'')">全部</li>
                    <?php foreach ($maintain as $v) { ?>
                        <li data-id="<?= $v['id'] ?>" class="maintain-li" onclick="maintain(this,<?= $v['id'] ?>)"><?= $v['nick_name'] ?></li>
                    <?php } ?>
                </ul>
            </div>
        </div>
            ...
            <div class="card-header">
                ...
                <div class="row">
                    ...
                    <div class="col-12 col-lg-3  form-group  col-sm-12 ">
                        <button type="button" id="add" class="btn btn-primary" onclick="add()">添加维修人员</button>
                    </div>
                </div>
            </div>
            <div class="card-body"">
...
<script>
    jQuery(document).ready(function($) {
        var url = '/maintain/ajax_list';
        var params = {};
        window.dataTable(params, url);
    });

    $('#name').change(function(){
        var url = '/maintain/list';
        var maintain=$(this).val().trim();
        window.location.href=url+'?maintain='+maintain
    });

    function maintain(is, id) {
        $(is).addClass('maintain-li-active')
        $('.maintain-li').not(is).removeClass('maintain-li-active')
        var url = '/maintain/ajax_list';
        var params = {
            'maintain_id': id
        };
    ...
    $('#search').click(function(){
        var url = '/maintain/ajax_list';
        var maintain_id=$('.maintain-li-active').attr('data-id')
